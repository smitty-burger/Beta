...
int board::Q_L()
{
	...
	//*/
	/*
	// For the Changed State Space Representation												TestG
	...
	// For dynamic changing of the Epsilon value while the script is running.
	...
	*/
	...
	else
	{
		...
		{
			// Reset Agent Position and Clear Movement History									TestE
			x_pos.clear();
			y_pos.clear();
			x_pos.push_front(start_pos[1]);
			y_pos.push_front(start_pos[2]);
...
void board::TestD()
{
	...
	for (int i = 0; i < max; i++)
	{
		for (int j = 0; j < max; j++)
		{
			for (int k = 0; k < 4; k++)
			{
				//cout << qtab[i][j][k] << endl;
				assert(qtab[i][j][k] < 100.001);
			}
		}
	};
}
...
void board::TestF()
{
	...
	// Find Minimum numbers of moves to get to the goal
	min_move = abs(goal_pos[1] - start_pos[1]) + abs(goal_pos[2] - start_pos[2]);
	//cout << min_move << endl;
	//Find the Lowest number of moves made by the Q-Learner
	auto min_sample = min_element(begin(score), end(score));
	//cout << min_sample[0] << endl;
	temp = min_sample[0];
	// Test minimum number of moves
	assert(temp == min_move);
